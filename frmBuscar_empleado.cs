        ...
        private void btn_aceptar_Click(object sender, EventArgs e)
        {
            ...
            if (dgv_buscemp.SelectedRows.Count == 1)
            {
                ...
                if (dgv_buscdir.SelectedRows.Count == 1)
                {
                    ...
                    DireccionSeleccionada = clsEmpleadodal.ObtenerDiremp(id2);

                    //jala el telefono            
                    if (dgv_busctel.SelectedRows.Count == 1)
                    {
                        int id3 = Convert.ToInt32(dgv_busctel.CurrentRow.Cells[0].Value);
                        TelefonoSeleccionado = clsEmpleadodal.ObtenerTelemp(id3);

                        if (dgv_buscor.SelectedRows.Count == 1)
                        {
                            int id4 = Convert.ToInt32(dgv_buscor.CurrentRow.Cells[0].Value);
                            CorreoSeleccionado = clsEmpleadodal.ObtenerCorreo(id4);
